...
class SimpleVirus(object):
    ...
    def reproduce(self, popDensity):
        ...
        n = random.random()
        if n > (self.maxBirthProb * (1 - self.popDensity)):
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for i in range(numTrials):
        ...
        for j in range(300):
            patient.update()
            virus_data.append(patient.getTotalPop())
...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        if all([self.isResistantTo(i) for i in self.activeDrugs]) == True:
            if n > self.maxBirthProb * (1 - popDensity):
                raise NoChildException
                ...
                for key in self.resistances.keys():
                    mutate_probability = random.random()

                    if mutate_probability > self.getMutProb():
...
class TreatedPatient(Patient):
    ...
    def addPrescription(self, newDrug):
        ...
        if not self.newDrug in self.prescriptions:
    ...
    def getResistPop(self, drugResist):
        ...
        for virus in self.viruses:
            if all([virus.isResistantTo(i) for i in self.drugResist]) == True:
                count += 1
        return count
    ...
    def update(self):
        ...
        population_density = len(self.getViruses()) / self.getMaxPop()

        viruses_2 = self.viruses[:]

        for i in viruses_2:
            try:
                i.reproduce(population_density, self.prescriptions)
...
if __name__ == "__main__":
    ...
    simulationWithDrug(1000, 1000, 0.7, 0.3, {"guttagonol":False}, 0.25, 10)
